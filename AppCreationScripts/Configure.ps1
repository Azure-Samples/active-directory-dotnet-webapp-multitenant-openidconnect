...
#> 
   ...
   Write-Host "Creating the AAD application (TodoListWebApp_MT)"
   # Get a 2 years application key for the service Application
   $pw = ComputePassword
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInYears 2 -pw $pw
   $serviceAppKey = $pw
   $serviceAadApplication = New-AzureADApplication -DisplayName "TodoListWebApp_MT" `
                                                   ...
                                                   -IdentifierUris "https://$tenantName/TodoListWebApp_MT" `
   ...
   Write-Host "Done creating the service application (TodoListWebApp_MT)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>TodoListWebApp_MT</a></td></tr>" -Path createdApps.html
